...
X, y = np.load('./data_preprocessed/ADNI1/data_fsaverage_nopvc_ico2.npy').astype('float32'), np.load('./data_preprocessed/ADNI1/labels.npy')
...
for i, task in enumerate(tasks):
    ...
    for train_idx, test_idx in kf.split(X_, y_):
        ...
        callbacks = [ModelSavingCallback(X_test, y_test, f"model_checkpoints/model_{task}_{fold}_adni1.h5", verbose=False),
